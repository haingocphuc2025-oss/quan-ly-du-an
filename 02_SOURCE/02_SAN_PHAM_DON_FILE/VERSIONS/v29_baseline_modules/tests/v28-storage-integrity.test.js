...
test('current module and handoff bundles are reproducible from the manifest', () => {
  const expected = buildFromManifest();
  const manifest = JSON.parse(fs.readFileSync(path.join(root, 'manifest.json'), 'utf8'));
  const modulePackage = fs.readFileSync(
    path.join(root, manifest.output), 'utf8'
  ).replace(/\r\n/g, '\n');
  const handoffPackage = fs.readFileSync(
    path.join(root, '..', 'BANDIAO', manifest.output), 'utf8'
  ).replace(/\r\n/g, '\n');
